(function (window, angular, undefined) {
  ...
  angular.module('gojira.lists', [
    ...
    'ListService',
    'UtilityService',
    function ListsCtrl($scope, $rootScope, $location, $dialog, AlertsService, ListService, UtilityService) {
      $scope.dialogOpts = {
        backdrop: true,
        keyboard: true,
        backdropClick: true,
    ...
    'ApiConfigService',
    'AuthService',
    function ListBoxCtrl($scope, $rootScope, dialog, AlertsService, ListService, ApiConfigService, AuthService) {
    ...
    'ApiConfigService',
    'AuthService',
    function MovieListBoxCtrl($scope, $rootScope, dialog, AlertsService, ListService, ApiConfigService, AuthService) {
    ...
    'ApiConfigService',
    'AuthService',
    function ListCtrl($scope, $rootScope, $location, $routeParams, $http, $dialog, $timeout, AlertsService, ListService, ApiConfigService, AuthService) {
    ...
    'AuthService',
    'AlertsService',
    function ListAddBoxCtrl($scope, $rootScope, $http, dialog, ListService, ApiConfigService, AuthService, AlertsService) {
  ...
  angular.module('gojira.movies', [
    ...
    'AuthService',
    'ListService',
    function MoviesCtrl($scope, $rootScope, $http, $dialog, $routeParams, ApiConfigService, RatingService, AlertsService, AuthService, ListService) {
      $scope.id = $routeParams.id;
      $scope.conf = ApiConfigService.getConf();
          ...
          $http.get(url, {}, { headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' } }).success(function (data, status) {
            ...
            if (data.credits.cast.length > 0) {
              ...
              angular.forEach(data.credits.cast, function (credit) {
                if (creditCount < 5) {
                  $scope.castMarkup[id] += '<div class="row-fluid"> <div class="span2"><img class="poster" src="' + $scope.imgUrl + '/w92/' + credit.poster_path + '"> </img></div>' + '<a href="#/movie/' + credit.id + '"><div class="span8 movie-name">' + credit.original_title + ' <i>( ' + credit.character + ' )</i></div></a></div>';
                }
                creditCount++;
              });
            }
      ...
      $scope.getBackground = function (url, path) {
        return { background: 'url( \'' + url + '/original/' + path + '\')' };
      };
      $scope.fetch = function () {
        $scope.imgUrl = $scope.conf.image.baseUrl;
  ...
  angular.module('gojira.ratings', [
    ...
    'AuthService',
    '$location',
    function RatingsCtrl($scope, $rootScope, $http, ApiConfigService, RatingService, AlertsService, AuthService, $location) {
      $scope.loaded = false;
      $scope.conf = ApiConfigService.getConf();
      $scope.userRatings = {};
      $scope.loadingClass = AlertsService.getLoadingClass();
  ...
  angular.module('gojira.search', [
    ...
    'AuthService',
    'ListService',
    function SearchCtrl($scope, $rootScope, $http, $dialog, ApiConfigService, RatingService, AlertsService, AuthService, ListService) {
      $scope.search = '';
      $scope.loaded = false;
      $scope.conf = ApiConfigService.getConf();
      $scope.userRatings = {};
  ...
  angular.module('gojira.user', [
    ...
    'AuthService',
    'ApiConfigService',
    function AuthCtrl($scope, $rootScope, $location, $http, $dialog, AlertsService, RatingService, AuthService, ApiConfigService) {
      $scope.dialogOpts = {
        backdrop: true,
        keyboard: true,
        backdropClick: true,
    ...
    'AuthService',
    'ApiConfigService',
    function LoginBoxCtrl($scope, $http, dialog, AlertsService, RatingService, ListService, AuthService, ApiConfigService) {
      $scope.name = '';
      $scope.pass = '';
      $scope.rName = '';
      $scope.rPass = '';
  ...
  ;
  angular.module('Alerts', []).factory('AlertsService', [
    '$rootScope',
    '$timeout',
  ...
  angular.module('Conf', []).factory('ApiConfigService', [
    '$http',
    function ($http) {
      var conf = {
          url: {
  ...
  angular.module('Auth', ['Storage']).factory('AuthService', [
    '$cookies',
    '$rootScope',
    'LocalStorageService',
    function ($cookies, $rootScope, LocalStorageService) {
  ...
  angular.module('List', ['Alerts']).factory('ListService', [
    '$http',
    '$rootScope',
    'AlertsService',
    'AuthService',
  ...
  angular.module('Storage', []).factory('LocalStorageService', function () {
    var store = function () {
        try {
  ...
  angular.module('Util', []).factory('UtilityService', [
    '$http',
    function ($http) {
      return {
